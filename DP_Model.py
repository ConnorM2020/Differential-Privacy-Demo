...
@app.route('/inject_target_user', methods=['GET'])
def inject_target_user():
    ...
    return jsonify({"status": "Injected target user successfully."})
...
def simulate_attack():
    try:
        ...
        visible_data = data.iloc[::-1].head(10)  # latest 10 injected first
        ...
        # Add DP noise (for simulation, not validation)
        ...
        # Match in noisy dataset (used to simulate attack success)
        ...
        for _, row in data.iterrows():
            ...
            if is_exact_match and is_noisy_match:
                risk_score = max(0.01, min(1.0, 1.0 - epsilon))
                vulnerable_list.append({
                    **record,
                    "status": "⚠️ Vulnerable (match despite noise)",
        ...
        if attack_type == "reid":
            ...
            match_set = data[
                (data['town'] == known_town) &
                (data['age'] == known_age) &
                (data['heart_rate'] == known_hr)
            ]
            known_age = int(known_age)
            known_hr = int(known_hr)

            member_hits = len(match_set)
            membership_probability = member_hits / len(data)
            inferred = "Likely Member" if membership_probability > 0.05 else "Not a Member"

            result_msg = (
                f"{member_hits} exact match(es) found in dataset. "
                f"Membership inference result: **{inferred}** (confidence: {membership_probability:.2%})."
            )

            matched_records = match_set.head(5).to_dict(orient='records')

        else:
            result_msg = "Invalid attack type selected."
        ...
        return jsonify({
            ...
            "match_confidence": round(membership_probability, 4) if membership_probability is not None else None,
            "match_count": member_hits if member_hits is not None else None,
            "dataset_size": len(data),
            "epsilon": epsilon
        })
...
def get_vulnerable_users_by_epsilon():
    ...
    visible_data = data.iloc[::-1].head(10).to_dict(orient='records')  # Latest 10 records

    vulnerable_users = []

    for user in visible_data:
        # Inject synthetic risk logic (or adapt to real logic)
        age = user.get("age", 0)
        hr = user.get("heart_rate", 0)
        risk_score = user.get("risk_score", 1.0)

        # Optional dynamic risk formula (example: normalized range)
        try:
            age_score = max(0, min(1, float(age) / 120))  # e.g. 0.5 for age 60
            hr_score = max(0, min(1, float(hr) / 200))    # e.g. 0.43 for HR 86
            risk_score = round((age_score + hr_score) / 2, 2)
        except:
            risk_score = 1.0  # fallback
